...
// === CONFIGURATION ===
// const parentDir = 'T:/book/20252206'; // Change if needed
 ...
 * @returns {boolean}
 */
const isLink8K = (link) => /\/8k\//i.test(link);
 ...
 * Now accepts a failureLogs array to collect errors.
 */
async function processLogFile(filePath, db, tableName, failureLogs, duplicatesTable) {
...
async function main() {
  ...
  const parentDir = 'T:/book/20250824';
  // const parentDir = 'T:/book/test';
  const dbPath = path.join(parentDir, '../gonow.db');
  const sqliteTableName = 'meta8k';
  // const inputFilePath = path.join(parentDir, 'SNH_Meta.log');
  const inputDir = parentDir; // Change if you want a different directory
  // const inputDir = 'T:/book/bak'; // Change if you want a different directory
  ...
  for (const file of files) {
    ...
    await processLogFile(absPath, db, sqliteTableName, failureLogs, duplicatesTable);
  }
